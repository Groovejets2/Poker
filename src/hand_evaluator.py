...
class HandEvaluator:
    ...
    def evaluate_best_hand(self, cards):
        ...
        For Texas Hold'em, finds the best 5-card combo from hole + community cards.
        
        Args:
            cards (list): List of 5+ Card objects
            
        Returns:
            dict: Same as evaluate() - the best 5-card hand
        ...
        # Try all 5-card combinations and pick the best
        from itertools import combinations
        best_eval = None
        best_strength = -1
        
        for combo in combinations(cards, 5):
            eval_result = self.evaluate(list(combo))
            if eval_result['strength'] > best_strength:
                best_strength = eval_result['strength']
